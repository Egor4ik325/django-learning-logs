...
class EntryCreateTests(TestCase):
    ...
    def test_positive(self):
        ...
        self.assertTrue(self.client.login(username="test", password="test"))
        self.url = reverse(self.url_name, kwargs={"topic_id": self.test_topic.id})
        response = self.client.post(
            self.url,
    ...
    def test_create_entry_for_unexisting_topic_authenticated(self):
        ...
        self.url = reverse(self.url_name, kwargs={"topic_id": last_topic.id + 1})
        response = self.client.post(
            self.url,
            {"text": "Create topic via API"},
        )
    ...
    def test_create_entry_for_other_user_topic_authenticated(self):
        self.assertTrue(self.client.login(username="test", password="test"))
        self.url = reverse(self.url_name, kwargs={"topic_id": self.other_topic.id})
        response = self.client.post(
            self.url,
            {"text": "Create topic via API"},
        )
